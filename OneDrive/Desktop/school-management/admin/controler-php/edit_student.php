<?php
require_once "select_single_student.php";
$error = [];
...
//image validation
    ...
    if(empty($dbPhoto["name"])){
     ...
     $imgPath = "uplode/profile/".$user['photo'];
...
//  uplode information
    ...
    if($uplode){
      ...
      $query = "UPDATE students_info SET name='$dbName',roll='$dbRol',class='$dbClass',city='$dbCity',p_contact='$dbPhone',photo='$photo_name' WHERE id= '$id' " ;

      $result = mysqli_query($connect,$query);
      if($result){
        $_SESSION['success'] = "<h5 class='alert alert-success'>Info Update Successfull!</h5>";
        header("location:index.php?page=dashboard");
      }else{
        $_SESSION['success'] = "<h5 class='alert alert-warning'>Updated feil!</h5>";
        header("location:../edit_student_view.php");
      }
    }
